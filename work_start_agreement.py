...
while True:
    ...
    logging.info(f"Checking if validator should be starting. This node is {node.identifier.hex}")
    agreement_info = pot.nodes.validator_agreement_info
    agreement_info.refresh()
    if not agreement_info.is_started and len(pot.nodes.validator_agreement_result.all()) == 0 and agreement_info.last_successful_agreement < time() - 139:
        ...
        def send(func):
            ...
            for node in pot.nodes.all():
                if node.identifier == pot.self_node.identifier or node.identifier not in pot.nodes.validators.all():
                    continue
                logging.info(f"Sending new validator agreement to host: {node.host}:{node.port} - starting thread")
